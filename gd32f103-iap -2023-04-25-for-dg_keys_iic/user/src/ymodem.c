...
static  int32_t Receive_Byte_from_uart (uint8_t *c, uint32_t timeout)
{
	while (timeout-- > 0)
	{
...
static  int32_t Receive_Byte_from_iic0 (uint8_t *c, uint32_t timeout)
{
	return i2c0_Receive(c, 1, timeout);
}
...
{
  	if(!is_cpu_update_cmd)  //表示从调试串口升级
	{
		return Receive_Byte_from_uart(c,timeout);
	}
	else
	{
		return Receive_Byte_from_iic0(c,timeout);
	}
}


static uint32_t Send_Byte_to_uart (uint8_t c)
{
	SerialPutChar(c);
	return 0;
}
...
static uint32_t Send_Byte (uint8_t c)
{
	if(!is_cpu_update_cmd)  //表示从调试串口升级
	{
		return Send_Byte_to_uart(c);
	}
	else
	{
		return Send_Byte_to_iic0(c);
	}
}



//static int32_t Receive_Packet_from_uart(uint8_t *data, int32_t *length, uint32_t timeout)
//{
...
//static int32_t Receive_Packet_from_iic(uint8_t *data, int32_t *length, uint32_t timeout)
//{
//	uint16_t i, packet_size;
//	uint8_t c;
//	uint8_t checksum;
//	*length = 0;
//	if (i2c0_Receive(&c, 1, timeout) != 0)
//	{
//		printf("i2c0_Receive timeout\r\n");
//		return -1;
//	}
////	printf("Receive_Packet c = %#x\r\n",c);
//	switch (c)
...
//			return 0;
//		case CA:
//			if ((i2c0_Receive(&c, 1, timeout) == 0) && (c == CA))
//			{
//				*length = -1;
//				return 0;
//			}
...
//	{
//	//	printf("for %d\r\n",packet_size + PACKET_OVERHEAD);
//		if (i2c0_Receive(data+i, 1, timeout) != 0)
//		{
//			printf("for Receive_Byte error i = %d\r\n",i);
//			return -1;
//		}
//		
//		if(i< packet_size + PACKET_HEADER) //最后的两个字节不参与校验和
//			checksum += data[i];   //计算校验和 
//	}
//	
//	if(checksum != data[packet_size + PACKET_HEADER])
//	{
//		printf("checksum error checksum = %#x,data[%d] = %#x\r\n",checksum,packet_size + PACKET_HEADER,data[packet_size + PACKET_HEADER]);
//		return -1;
//	}
//	
//	if (data[PACKET_SEQNO_INDEX] != ((data[PACKET_SEQNO_COMP_INDEX] ^ 0xff) & 0xff))
...
*********************************************************************************************************
*	oˉ êy ??: UpdateCRC16
*	1|?ü?μ?÷: é?′?????μ?CRC?á1? crcIn ?ù?óé?ò???×??úêy?Y????CRC
*	D?    2?: crcIn é?ò?′?CRC?????á1?
*             byte  D?ìí?ó×??ú
*	·μ ?? ?μ: ?T
*********************************************************************************************************
*/
...
//	checksum = c;
	...
	if(is_cpu_update_cmd)  //从cpu升级，iic的方式
	{
...
//							else if((packet_data[PACKET_SEQNO_INDEX]) == 0 && 1 == (packets_received & 0xff))
//							{
//								continue;
//							}
							else
