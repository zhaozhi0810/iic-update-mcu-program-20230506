...
void get_cmd_from_debug_uart0(void)
{
	...
	if(SerialKeyPressed_Uart0((uint8_t*)&key))
	{
		if (key == '1')
		{
			...
			if(0==SerialDownload())
			{
				printf("\rdebug uart SerialDownload done!\r\n");
				if(!mcu_download_done())
				{
					printf("\rdebug uart mcu_download_done!\r\n");
...
//					{
//						checksum = (uint8_t)(0xa5+0xa5);
//						SerialPutChar_uart1(0x55);
//						SerialPutChar_uart1(0x55);
...
//0x55 0xaa 0x90 0x0/0x1 checksum

void get_cmd_from_rk3399_iic0(void)
{
	uint8_t key,i;
	uint8_t *pflash_md5 = (void*)(UPDATE_FLAG_START_ADDR + DOWN_MD5_OFFET);
	static uint8_t buf[8];
	...
	if(i2c0_Receive_byte((uint8_t*)&key))
	{		
		if(key == 0x55)
			index = 0;
		...
		if(index < 5)
		{
			...
			if(index == 5)
			{
				...
				if(buf[1] == 0xaa && buf[2] == 0xd0)   //0xd0 是升级命令
				{	
				//	set_download_uart(1); //通讯串口
					if((buf[3] == 0) && (buf[4] == 0xcf))  //是升级命令
					{
						...
						for (i = 0; (i < FILE_MD5_LENGTH);i++)
						{
							Send_Byte_to_iic0(pflash_md5[i]);
							//printf("pflash_md5[%d] = %#x\r\n",i,pflash_md5[i]);
							checksum += pflash_md5[i];							
						}
						Send_Byte_to_iic0(checksum);  //总共35个字节！！！！
					//	printf("(buf[3] == 0) && (buf[4] == 0xcf) checksum = %d\r\n",checksum);
					}
					else if((buf[3] == 1) && (buf[4] == 0xd0))  //上位机确认需要下载更新
					{
					//	printf("(buf[3] == 1) && (buf[4] == 0xd0)\r\n");
...
//	uint8_t key = 0;
	...
	while (1)
	{
		...
		get_cmd_from_rk3399_iic0();  //从通讯串口（rk3399）判断是否更新。
	}
}
